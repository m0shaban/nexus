...
interface Note {
  ...
  user_id?: string
  raw_telegram_message?: object | null
}
...
export function NotesDisplay() {
  ...
  const [statusFilter, setStatusFilter] = useState('all')  // جلب الملاحظات
  const fetchNotes = useCallback(async () => {
    try {
      ...
      try {
          ...
          setNotes([
            {
              ...
              content: 'مرحباً بك في Nexus! هذه ملاحظة تجريبية.',
              content_type: 'text',
              analysis_status: 'completed' as const,
              ai_summary: 'ملاحظة ترحيبية تجريبية',
              ai_questions: ['كيف يمكنني البدء؟'],
              user_id: 'demo-user'
            },
            {
              id: '2', 
              ...
              content: 'لإعداد قاعدة البيانات، يرجى تشغيل سكريبتات SQL في Supabase',
              content_type: 'text',
              analysis_status: 'pending' as const,
              ai_summary: 'تعليمات إعداد قاعدة البيانات',
              ai_questions: ['ما هي الخطوات المطلوبة؟'],
              user_id: 'demo-user'
            }
          ])
          setLoading(false)
        ...
        // استخدام بيانات تجريبية
        setNotes([])
        alert('قاعدة البيانات غير متصلة. يرجى إعداد Supabase أولاً.')
        setLoading(false)
        return
      }
      ...
      if (response.ok) {
        ...
        alert('فشل في تحميل الملاحظات: ' + (errorData.error || 'خطأ غير معروف'))
      }    } catch (error) {
      console.error('Error fetching notes:', error)
      alert('خطأ في الشبكة أو الخادم عند تحميل الملاحظات')
    } finally {
      setLoading(false)
    }
  }, [statusFilter])
